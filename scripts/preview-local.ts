...
async function main() {
  ...
  try {
    ...
    const p1 = await prisma.produto.findFirst({ where: { nome: 'Açúcar' } });
    const p2 = await prisma.produto.findFirst({ where: { nome: 'Arroz Parbolizado' } });
    if (p1 && p2) {
      const it = (p: { id: string; unidadeId: string }, q: number) => ({ produtoId: p.id, unidadeId: p.unidadeId, quantidade: q });
      await criarMovimento('entrada', { data, itens: [it(p1, 10), it(p2, 12)] }, session);
      ...
      await criarPedido({ data, itens: [it(p1, 20)] }, session);
      console.log('  lancamentos de exemplo criados (1 entrada, 1 saida, 1 descarte, 1 pedido).');
    }
