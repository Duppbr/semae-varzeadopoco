...
export default async function SaidaPdfPage({ params }: Props) {
  ...
  const half = Math.ceil(saida.itens.length / 2);
  ...
  const css = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page { max-width: 820px; margin: 20px auto; padding: 28px 32px; background: white; position: relative; border-radius: 8px; box-shadow: 0 1px 8px rgba(0,0,0,0.08); }
    .watermark { position: fixed; top: 50%; left: 50%; transform: translate(-50%,-50%); width: 280px; height: 280px; background-image: url('/logo-semae.png'); background-size: contain; background-repeat: no-repeat; background-position: center; opacity: 0.04; z-index: 0; pointer-events: none; }
    ...
    /* Duas colunas */
    .items-cols { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 20px; }
    ...
    /* Total no rodapé das colunas */
    .items-total { text-align: right; font-size: 10px; font-weight: 700; color: #475569; margin-bottom: 16px; margin-top: -14px; }
  ...
  const colTable = (items: typeof saida.itens, offset: number) => (
    <table className="col-table">
      <thead>
        <tr>
          ...
          <th className="num" style={{ width: '15%' }}>Un.</th>
        </tr>
      </thead>
      <tbody>
        {items.map((it, idx) => (
            ...
            {useTwoCols ? (
              <>
                <div className="items-cols">
                  {colTable(saida.itens.slice(0, half), 0)}
                  {colTable(saida.itens.slice(half), half)}
                </div>
                <p className="items-total">Total: {saida.itens.length} produto(s)</p>
              </>
            ) : (
              <table>
                <thead>
